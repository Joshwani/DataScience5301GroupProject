---
title: "DTSC 5301 Group Project: Data Science for Finace: Tactical Asset Allocation with a Bubble Signal"
---
...
library('ggplot2')
library('dplyr')
POS_BUBBLE_THRESHOLD = 0.15 # explained later in notebook
NEG_BUBBLE_THRESHOLD = 0.15 # explained later in notebook
```
...
df <- read.csv('data/universe.csv', row.names = 'Date')
head(df)
```
...
The first step is to compute the bubble indicators for the benchmark. Here we are importing the signal pre-computed from Boulder Investment Technologies GitHub Repository. This data has been de-trended prior to analysis so as to obtain a evenly distributed number of positive and negative bubbles.

### Load Bubble Data

```{r}
bubble_df<- read.csv('data/confidence.csv', row.names='time')
bubble_df$Date <- as.Date(rownames(bubble_df))

# create unified conf score as pos_cong-neg_conf
...
ggplot(m_df, aes(x=Date)) + 
  ...
  labs(x="Date", y="Cumulative Returns Benchmark (red)\nvs\nBubble Portfolio (green)")
```
